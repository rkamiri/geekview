...
$db = new PDO('mysql:host=localhost;dbname=u434989051_dazy;charset=utf8', 'u434989051_dazy', 'tactac', [
...
function getMessages(){
  global $db;
  // 1. On requête la base de données pour sortir les 15 derniers messages
...
function postMessage(){
  ...
  if(!array_key_exists('author', $_POST) || !array_key_exists('content', $_POST)){
    ...
    $tup=array('author'=>$_POST['author'],'content'=>htmlspecialchars($_POST['content']));


	

  // 2. Créer une requête qui permettra d'insérer ces données
    $req = $db->prepare('insert into messages values(default, :author, :content, now());');
	$req->execute($tup);
